...
#define	INIT_PORT()     initUsartSD()//SDCard_init()       /* Initialize MMC control port */
#define FAST_MODE()     SDCard_fastMode()   /* Maximize SD Card transfer speed */
#define DLY_US(n)       delay_us(n)  // Delay n microseconds           // KLQ

#define	CS_H()          SD_CS_HI//SDCard_setCSHigh()  /* Set MMC CS "high" */
#define CS_L()          SD_CS_LOW//SDCard_setCSLow()   /* Set MMC CS "low" */
...
//#endif
    ...
    s = disk_status_mmc(drv);        /* Check if card is in the socket */
...
)
{
    ...
    if (disk_status_mmc(drv) & STA_NOINIT)                    /* Check if card is in the socket */
        return RES_NOTRDY;
...
uint8_t detectCard(void)
{
  ...
  if(validateCSD())
  {
    disk_status_mmc(0); // Update the INS variable
  ...
  disk_initialize_mmc(0);  // Attempt to initialize it
