...
//typedef void (*BrowserOpen)();

typedef void (*MoveCursor)(
	int posX, 
	int posY, 
	int type,
	int temeMs
...
typedef void (*ClickLeft)(
	int delayMs
);

//скрол
typedef void (*MoveScroll)(
	int scroll,
	int timeMs
);
...
int main(void) {
	...
	BOOL freeResult, runTimeLinkSuccess = FALSE; 
   	...
   	MoveCursor MoveCursorPtr = NULL;
   	ClickLeft ClickLeftPtr = NULL;
   	MoveScroll MoveScrollPtr = NULL;
	dllHandle = LoadLibrary("uiDll.dll");
	// If the handle is valid, try to get the function address. 
   	if (NULL != dllHandle){ 
      	//Get pointer to our function using GetProcAddress:
      	MoveCursorPtr = (MoveCursor)GetProcAddress(dllHandle,"MoveCursor");
      	ClickLeftPtr = (ClickLeft)GetProcAddress(dllHandle,"ClickLeft");
      	MoveScrollPtr = (MoveScroll)GetProcAddress(dllHandle,"MoveScroll");

      	// If the function address is valid, call the function. 
      	if (runTimeLinkSuccess = (NULL != MoveCursorPtr)){
        	ClickLeftPtr(1);
        	MoveScrollPtr(1,200);
        	MoveCursorPtr(1,1,0,200);
        	ClickLeftPtr(1);
        	MoveScrollPtr(1,200);
        	MoveCursorPtr(30,30,0,200);
        	ClickLeftPtr(1);
        	MoveScrollPtr(1,200);
        	MoveCursorPtr(300,300,0,200);
    	}
    	else
   			printf("runTimeLinkSuccess null\n");    		

      	//Free the library:
      	freeResult = FreeLibrary(dllHandle);       
   	}
   	else{
   		printf("dllHandle null\n");
   	}
...
//			printf("Error %d thread %u\n",nThread,WSAGetLastError());
		...
		if(len>0){
			...
			tcp.data = (uint8_t*)pBuf;
			trains[iTrain].command = COMMAND_READ;
			trains[iTrain].request = (uint8_t*)pBuf;
			trains[iTrain].requestLen  = len;
